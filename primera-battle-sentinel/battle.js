(() => {
  ...
  const S = { battles:new Map(), frames:new Map(), alerts:loadAlerts(), busy:false, status:'Starting…' };
  const P = loadPrefs();

  function loadAlerts(){ try{return JSON.parse(localStorage.getItem(STORE)||'{}')}catch{return{}} }
  function saveAlerts(){
    const now=Date.now();
  ...
  function loadPrefs(){
    try{return Object.assign({silent:false,hidden:false,minimized:false},JSON.parse(localStorage.getItem(PREF_STORE)||'{}'))}
    catch{return{silent:false,hidden:false,minimized:false}}
  }
  ...
  function intel(d){
    ...
    const vals=[];for(const e of d.querySelectorAll('[id*="damage" i],[class*="damage" i]'))vals.push(...nums(e.textContent));
    ...
    if(!Number.isFinite(out.leftBH)||!Number.isFinite(out.rightBH)){
      const top=[];for(const box of d.querySelectorAll('table,div')){
  ...
  function read(id,force=false){
    ...
    try{
      ...
      const s=timer(d);if(Number.isFinite(s)){b.seconds=s;b.error='';b.round=roundKey(d,id)}else if(force)b.error='Timer not found';
      b.intel=intel(d);
      if(s>0&&s<=ALERT_AT&&!S.alerts[b.round]){S.alerts[b.round]=Date.now();saveAlerts();alertBattle(b)}
    }catch{b.error='Frame read error'}
  }

  function alertBattle(b){
    const i=b.intel||{},bh=(Number.isFinite(i.leftBH)||Number.isFinite(i.rightBH))?` • BH ${compact(i.leftBH)} / ${compact(i.rightBH)}`:'';
    const title=`⚔ ${b.name}`,text=`${time(b.seconds)} left${bh}`;
    if(!P.silent){
      beep();
      if(typeof GM_notification==='function'){try{GM_notification({title,text,timeout:15000,onclick:()=>{window.focus();if(!P.hidden&&!P.minimized)flash(b.id)}})}catch{}}
      else if('Notification'in window&&Notification.permission==='granted'){
        const n=new Notification(title,{body:text,tag:`pbs-${b.id}`});n.onclick=()=>{window.focus();if(!P.hidden&&!P.minimized)flash(b.id);n.close()};
      }
    }
    render();
    if(!P.hidden&&!P.minimized)flash(b.id);
  }
  function beep(){
    if(P.silent)return;
  ...
  function travel(id){
    ...
    if(x.length!==1)return fail(id,x.length?'Multiple travel controls':'Travel control not found');x[0].click();toast(`Clicked: ${label(x[0])}`);setTimeout(()=>reload(id),1500);
  }
  function hit(id){
    const d=frameDoc(id);if(!d)return fail(id,'Battle frame not ready');
    ...
    if(x.length!==1)return fail(id,x.length?'Two fight sides detected':'Single-hit control not detected');x[0].click();toast(`1 HIT sent: ${label(x[0])}`);
  }
  function fail(id,m){open(id);toast(`${m}. Opened battle.`)}

  function applyVisibility(){
    const root=document.getElementById('pbs3'),show=document.getElementById('pbs3-show');
    if(root)root.style.display=P.hidden?'none':'';
    if(show)show.style.display=P.hidden?'block':'none';
    const body=document.getElementById('pbs3-body'),min=document.getElementById('pbs3-min');
    if(body)body.style.display=P.minimized?'none':'';
    if(min)min.textContent=P.minimized?'+':'−';
    const silent=document.getElementById('pbs3-silent');
    if(silent){silent.textContent=P.silent?'🔇':'🔊';silent.title=P.silent?'Silent mode ON. Click to enable alerts.':'Alerts ON. Click for silent mode.';silent.style.background=P.silent?'#704040':'#2f5f42'}
  }
...
#pbs3-show{position:fixed;left:0;bottom:18px;z-index:2147483647;border-radius:0 8px 8px 0;padding:8px 10px;background:#232a32;display:none}
.pbs-refill{background:#2a6541!important}.pbs-travel{background:#355d87!important}.pbs-hit{background:#8a3030!important}.pbs-hide{background:#55343a!important}
...
#pbs3-flash{animation:pbsflash .35s alternate 8}@keyframes pbsflash{from{box-shadow:0 0 0 #0000}to{box-shadow:0 0 24px #ff4242}}`;
    ...
    const r=document.createElement('div');r.id='pbs3';r.innerHTML=`<div id="pbs3-head"><div><b>⚔ Battle Sentinel</b> <span class="pbs-small">v3.3</span></div><div><button id="pbs3-silent" title="Toggle silent mode">🔊</button><button id="pbs3-notify" title="Request desktop notification permission">🔔</button><button id="pbs3-refresh" title="Refresh battles">↻</button><button id="pbs3-hide" class="pbs-hide" title="Hide until you bring it back">✕</button><button id="pbs3-min" title="Minimize">−</button></div></div><div id="pbs3-body"><div id="pbs3-status" class="pbs-small">Starting…</div><div id="pbs3-list"></div><div class="pbs-small">DMG = side total. BH = current top damage detected for each side.</div></div><div id="pbs3-toast"></div>`;
    document.body.appendChild(r);

    const show=document.createElement('button');show.id='pbs3-show';show.textContent='⚔ SHOW';show.title='Bring Battle Sentinel back';document.body.appendChild(show);
    ...
    document.getElementById('pbs3-silent').onclick=()=>{P.silent=!P.silent;savePrefs();applyVisibility();toast(P.silent?'Silent mode ON':'Alerts ON')};
    ...
    document.getElementById('pbs3-hide').onclick=()=>{P.hidden=true;savePrefs();applyVisibility()};
    document.getElementById('pbs3-min').onclick=()=>{P.minimized=!P.minimized;savePrefs();applyVisibility()};
    show.onclick=()=>{P.hidden=false;savePrefs();applyVisibility()};
    applyVisibility();
  }
  ...
  function render(){
    ...
    st.textContent=`${S.status} • timers ${a.filter(x=>Number.isFinite(x.seconds)).length}/${a.length}${P.silent?' • SILENT':''}`;
  ...
  function flash(id){if(P.hidden||P.minimized)return;const c=document.querySelector(`[data-card="${CSS.escape(String(id))}"]`);if(!c)return;c.id='pbs3-flash';c.scrollIntoView({behavior:'smooth',block:'center'});setTimeout(()=>{if(c.id==='pbs3-flash')c.removeAttribute('id')},3500)}
